...
class Outils(ABC):
    '''Cette classe regroupe les méthodes nécessaires à la bonne exécution de certaines fontions, notamment transforme.json_data
    ...
    --------
    fusion:
        Concaténne deux listes en gardant leur ordre et en supprimant les doublons.
        
    clefs_dictionnaire:
        Renvoie la liste des clés d'un dictionnaire
        
    clefs_dictionnaire_imbrique: recursive
        Renvoie deux listes. La liste de toutes les clées à tous les étages, et la liste des clés qui sont associées à un dictionnaire.
    ...
    def clef_dict_imbrique(data, clef_passage):
        ...
        >>>
        '''
        clef=Outils.clefs_dictionnaire(data)
        sous_clef=[]
        for i in clef :
            if isinstance(data[i], dict):
                clef_passage.append(i)
                sous_clef=Outils.fusion(sous_clef, Outils.clef_dict_imbrique(data[i], clef_passage)[0])
        return (Outils.fusion(clef, sous_clef), clef_passage)